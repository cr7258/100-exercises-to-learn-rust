...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(e) => match e {
            // TicketNewError::TitleError(e) in a match expression means match and destructure:
            // it matches the TitleError variant and binds the inner String value to the variable e.
